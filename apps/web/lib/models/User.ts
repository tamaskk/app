...
const UserSchema = new Schema(
  {
    ...
    // One sub-document, denormalised so we never need a join to render the
    // paywall state on the dashboard. The platform-specific receipt is kept
    // around for re-validation but never returned to clients.
    //
    // Status machine:
    //   free       → never paid, may not have used a trial yet
    //   trialing   → 3-day free trial in flight; access ends at trialEndsAt
    //   active     → paid; access continues until currentPeriodEnd
    //   cancelled  → user cancelled but currentPeriodEnd is in the future
    //                (they still get Pro until then)
    //   expired    → access window has passed; treated like free for gating
    subscription: {
      type: new Schema(
        {
          status: {
            type: String,
            enum: ['free', 'trialing', 'active', 'cancelled', 'expired'],
            default: 'free',
            index: true,
          },
          // null while free / trialing-only.
          plan: { type: String, enum: [null, 'monthly', 'yearly'], default: null },
          // null until the paywall is confirmed.
          provider: {
            type: String,
            enum: [null, 'apple', 'google', 'stripe', 'manual'],
            default: null,
          },
          // App Store / Play Store product identifier (e.g.
          // `heftor_pro_monthly`). Useful when the same plan is sold under
          // multiple SKUs across regions.
          productId: { type: String, default: null },
          // One trial per account — once trialUsedAt is set the user
          // cannot start a second trial even after cancelling.
          trialUsedAt: { type: Date, default: null },
          trialEndsAt: { type: Date, default: null },
          currentPeriodEnd: { type: Date, default: null },
          // Last successful receipt validation. We treat the user as Pro
          // for `currentPeriodEnd` regardless, but this helps debug
          // store-vs-server drift.
          lastValidatedAt: { type: Date, default: null },
          // Raw receipt blob (Apple unified receipt or Play purchase
          // token). Never returned to the client.
          rawReceipt: { type: String, default: null, select: false },
        },
        { _id: false },
      ),
      default: () => ({ status: 'free' }),
