...
const SetSchema = new Schema(
  {
    ...
    // HYROX metric fields (optional, additive) — see Training.ts for the
    // rationale. Logged so the summary screen can show "1000 m · 4:12" etc.
    distanceM: { type: Number, default: null },
    seconds: { type: Number, default: null },
    targetKg: { type: Number, default: null },
    // Logged time for the set (HYROX station records / time PRs).
...
const SessionExerciseSchema = new Schema(
  {
    ...
    // HYROX metric (null → "reps") — kept so PR detection can skip non-strength
    // stations and the summary can render the right units.
