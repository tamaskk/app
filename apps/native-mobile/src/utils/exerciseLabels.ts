// Ported 1:1 from apps/mobile/lib/utils/exercise_labels.dart.
//
...
import { isHu } from "../i18n";

const en2hu: Record<string, string> = {
  abdominals: "hasizom",
...
export function exLabel(enValue: string): string {
  if (!isHu()) return enValue;
  return en2hu[enValue.toLowerCase().trim()] ?? enValue;
}

/// Localize a list of catalogue terms (e.g. targetMuscles) for display.
export function exLabels(enValues: Iterable<string>): string[] {
  return [...enValues].map(exLabel);
}
